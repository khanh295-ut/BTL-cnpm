# backend/src/presentation/routes/expert_routes.py
...
def get_expert(expert_id: UUID, db: Session = Depends(get_db)):
    expert = expert_service.get_by_id(db, expert_id)
    if expert is None:
...
def create_expert(data: ExpertCreate, db: Session = Depends(get_db)):
    return expert_service.create(db, data)

@router.put("/{expert_id}", response_model=ExpertResponse)
def update_expert(expert_id: UUID, data: ExpertUpdate, db: Session = Depends(get_db)):
    expert = expert_service.update(db, expert_id, data)
    if expert is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expert not found")
    return expert
...
def delete_expert(expert_id: UUID, db: Session = Depends(get_db)):
    deleted = expert_service.delete(db, expert_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expert not found")
    return None
